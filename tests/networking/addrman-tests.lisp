...
(defun %ip (a b c d) (bl.net:ipv4-to-mapped-ipv6 a b c d))

(defun %now () (bl.ser:get-unix-time))
...
(test net-group-key-ipv4-is-slash16
  (is (equal '(1 203 0)
             (coerce (bl.net:net-group-key (%ip 203 0 113 5)) 'list))))
...
(test buckets-deterministic-and-in-range
  (let* ((book (%ab)) (pa (%pa 1 2 3 4))
         (sg (bl.net:net-group-key (%ip 9 9 9 9))))
    (is (= (bl.net::new-bucket book pa sg)
           (bl.net::new-bucket book pa sg)))
    (is (<= 0 (bl.net::new-bucket book pa sg) 1023))
    (is (= (bl.net::tried-bucket book pa)
           (bl.net::tried-bucket book pa)))
    (is (<= 0 (bl.net::tried-bucket book pa) 255))
    (is (<= 0 (bl.net::bucket-position book pa t 0) 63))))
...
(test add-creates-new-entry
  (let ((book (%ab)))
    ...
    (let ((pa (bl.net:address-book-lookup book (%ip 1 2 3 4) 8333)))
      ...
      (is (null (bl.net:peer-address-in-tried pa))))))

(test add-rejects-unroutable
  (let ((book (%ab)))
    (is (null (bl.net:address-book-add
...
(test good-promotes-new-to-tried
  (let ((book (%ab)))
    ...
    (let ((pa (bl.net:address-book-lookup book (%ip 1 2 3 4) 8333)))
      ...
      (is (= 0 (bl.net:peer-address-ref-count pa))))))

(test attempt-counts-failures
  (let ((book (%ab)))
    (bl.net:address-book-add book (%pa 1 2 3 4))
    (bl.net:address-book-attempt book (%ip 1 2 3 4) 8333 :count-failure t)
    (let ((pa (bl.net:address-book-lookup book (%ip 1 2 3 4) 8333)))
      (is (= 1 (bl.net:peer-address-n-attempts pa))))))

;;;; Quality

(test terrible-when-ancient
  (is-true (bl.net:addr-info-terrible-p (%pa 1 2 3 4 :last-seen 1000) (%now))))

(test not-terrible-when-recent
  (let ((now (%now)))
    (is (null (bl.net:addr-info-terrible-p (%pa 1 2 3 4 :last-seen now) now)))))

;;;; Select / GetAddr

(test select-returns-known-address
  (let ((book (%ab)))
    (dotimes (i 5) (bl.net:address-book-add book (%pa 10 0 0 i)))
    (is (not (null (bl.net:address-book-select book))))))

(test select-empty-book-is-nil
  (is (null (bl.net:address-book-select (%ab)))))

(test get-addr-skips-terrible
  (let ((book (%ab)))
    (bl.net:address-book-add book (%pa 10 0 0 1 :last-seen (%now)))
    (bl.net:address-book-add book (%pa 10 0 0 2 :last-seen 1000)) ; ancient
    (let ((addrs (bl.net:address-book-get-addr book :max 0 :pct 100)))
      (is (= 1 (length addrs)))
      (is (equalp (%ip 10 0 0 1)
                  (bl.net:peer-address-ip (first addrs)))))))

(test get-addr-respects-max
  (let ((book (%ab)))
    (dotimes (i 20) (bl.net:address-book-add book (%pa 12 0 0 i)))
    (is (<= (length (bl.net:address-book-get-addr book :max 5 :pct 100)) 5))))
...
1024 new buckets — an attacker flooding from one source can poison only 64."
  (let* ((book (%ab))
         (sg (bl.net:net-group-key (%ip 6 6 6 6)))
         (buckets (make-hash-table)))
    (dotimes (i 600)
      (let ((pa (%pa (+ 11 (mod i 200)) (mod (floor i 200) 256) (mod i 251) (mod i 241))))
        (setf (gethash (bl.net::new-bucket book pa sg) buckets) t)))
...
(defun %am-hex (s) (bl.crypto:hex-to-bytes s))
...
AddrMan::Add(…, source) with a Tor source."
  (let* ((book (%ab))
         (sg (bl.net:net-group-key (%am-hex +am-onion-pk+) :torv3)))
    (is-true (bl.net:address-book-add book (%pa 1 2 3 4) sg))
    (let ((pa (bl.net:address-book-lookup book (%ip 1 2 3 4) 8333)))
      (is (not (null pa)))
      (is (equalp sg (bl.net::peer-address-source-group pa))))))
...
TORv3 pubkey that happens to equal an I2P hash can never collide in the map."
  (let ((bytes (%am-hex +am-onion-pk+)))
    (is (not (equalp (bl.net:make-address-key bytes 8333 :torv3)
                     (bl.net:make-address-key bytes 8333 :i2p))))
    ;; And the same record keys identically both times.
    (is (equalp (bl.net:make-address-key bytes 8333 :torv3)
                (bl.net:make-address-key bytes 8333 :torv3)))
    ;; NIL net derives ipv4/ipv6 from 16-byte mapped form (compat callers).
    (is (equalp (bl.net:make-address-key (%ip 1 2 3 4) 8333)
                (bl.net:make-address-key (%ip 1 2 3 4) 8333 :ipv4)))))
...
(test same-bytes-different-net-coexist
  ...
  (let ((book (%ab))
    ...
    (let ((tor (bl.net:address-book-lookup book bytes 8333 :torv3))
          (i2p (bl.net:address-book-lookup book bytes 8333 :i2p)))
      (is (not (null tor)))
      (is (not (null i2p)))
      (is (not (eq tor i2p)))
      (is (eq :torv3 (bl.net:peer-address-network tor)))
      (is (eq :i2p (bl.net:peer-address-network i2p))))))

(test addrman-good-promotes-onion
  "Good() works for a typed record: the onion entry moves new -> tried."
  (let ((book (%ab))
        (bytes (%am-hex +am-onion-pk+)))
    (bl.net:address-book-add book (%pa-net :torv3 bytes))
    (is-true (bl.net:address-book-good book bytes 8333 (%now) :torv3))
    (let ((pa (bl.net:address-book-lookup book bytes 8333 :torv3)))
      (is (not (null pa)))
      (is-true (bl.net:peer-address-in-tried pa)))))
  ...
  (loop for i from 1 to limit
        do (let ((pa (%pa (ldb (byte 8 16) i) (ldb (byte 8 8) i)
             ...
             (bl.net:address-book-good
              book (bl.net:peer-address-ip pa) 8333 (%now) :ipv4)
  ...
  (let ((book (bl.net:make-address-book))
    ...
    (unwind-protect
         (let ((path (merge-pathnames "peers.dat" tmp-dir))
           ...
           (bl.net:address-book-add book (%pa 1 2 3 4 :services 9))
           (bl.net:address-book-add
            book (bl.net:make-peer-address
           ...
           (bl.net:address-book-add book (%pa-net :torv3 onion))
           (bl.net:address-book-add book (%pa-net :i2p i2p :port 0))
           (bl.net:address-book-add book (%pa-net :cjdns cjdns))
           (bl.net:address-book-good book onion 8333 (%now) :torv3)
           ...
           (let ((book2 (bl.net:make-address-book)))
             ...
             (let ((tor (bl.net:address-book-lookup book2 onion 8333 :torv3)))
               (is (not (null tor)))
               (is (eq :torv3 (bl.net:peer-address-network tor)))
               (is-true (bl.net:peer-address-in-tried tor)))
             (let ((v4 (bl.net:address-book-lookup book2 (%ip 1 2 3 4) 8333)))
               (is (not (null v4)))
               (is (= 9 (bl.net:peer-address-services v4))))
             (is (not (null (bl.net:address-book-lookup book2 i2p 0 :i2p))))
             (is (not (null (bl.net:address-book-lookup book2 cjdns 8333 :cjdns))))))
         ...
         (let ((path (merge-pathnames "peers.dat" tmp-dir)))
           ...
           (let ((book (bl.net:make-address-book)))
             ...
             (let ((a (bl.net:address-book-lookup book (%ip 1 2 3 4) 8333)))
               ...
               (is (= 9 (bl.net:peer-address-services a)))
               (is-true (bl.net:peer-address-in-tried a)))
             (let ((b (bl.net:address-book-lookup book (%ip 5 6 7 8) 18333)))
               (is (not (null b)))
               (is (= 1700000100 (bl.net:peer-address-last-seen b))))
             (let ((c (bl.net:address-book-lookup
...
(test get-addr-includes-typed-records
  ...
  (let ((book (%ab)))
    ...
    (let ((addrs (bl.net:address-book-get-addr book :pct 100)))
      (is (= 1 (length addrs)))
