...
the 4-byte SIGNET_HEADER when present)."
  (let ((base (%sig-cat (%sig-bv #x6a #x24 #xaa #x21 #xa9 #xed) segwit-hash32)))
    (if signet-payload
        (%sig-cat base (bl.val::%minimal-push signet-payload))
        base)))
...
any solution check -- Core's genesis short-circuit."
  (let* ((cb (%sig-coinbase (%sig-witness-commitment (%sig-fill 32 0))))
         (blk (%sig-block cb))
         (h (bl.ser:block-header-hash
             (bl.ser:bitcoin-block-header blk))))
    (is-true (bl.val:check-signet-block-solution blk (%sig-bv #x51) h))))
...
built and the empty scriptSig satisfies OP_TRUE (Core allows this)."
  (let* ((cb (%sig-coinbase (%sig-witness-commitment (%sig-fill 32 0))))
         (blk (%sig-block cb)))
    (is-true (bl.val:check-signet-block-solution
...
(test signet-no-witness-commitment-rejected
  ...
  (let* ((cb (%sig-coinbase (%sig-bv #x6a #x00)))   ; bare OP_RETURN, not a commitment
         (blk (%sig-block cb)))
    (is-false (bl.val:check-signet-block-solution
               blk (%sig-bv #x51) +sig-dummy-genesis+))))
...
data encountered')."
  (let* ((payload (%sig-cat bl.val::*signet-header*
...
(test signet-challenge-for-network
  ...
  (is (equalp bl.val:*default-signet-challenge*
              (bl.val:signet-challenge-for-network :signet)))
  (is (null (bl.val:signet-challenge-for-network :mainnet)))
  (is (null (bl.val:signet-challenge-for-network :testnet4))))
...
parse/clear, and the legacy sighash all lining up."
  (let* ((privkey (let ((k (%sig-fill 32 0))) (setf (aref k 31) 1) k))
         (pubkey (bl.crypto:derive-public-key privkey))         ; 33-byte compressed
         ...
         (cb0 (%sig-coinbase (%sig-witness-commitment
                              (%sig-fill 32 0) bl.val::*signet-header*)))
    ...
    (multiple-value-bind (to-spend to-sign)
      ...
      (let* ((sighash (bl.interop::compute-legacy-sighash
             ...
             (script-sig (bl.val::%minimal-push full-sig))
             (solution (%sig-serialize-solution script-sig '()))
             (payload (%sig-cat bl.val::*signet-header* solution))
        ...
        (let* ((bad (copy-seq full-sig)))
          ...
          (let* ((bad-ss (bl.val::%minimal-push bad))
                 (bad-sol (%sig-serialize-solution bad-ss '()))
                 (bad-payload (%sig-cat bl.val::*signet-header* bad-sol))
...
this tree already records in chain.lisp."
  (is (string= "00000377ae000000000000000000000000000000000000000000000000000000"
               (string-downcase
                (format nil "~64,'0x" bl.store:+signet-pow-limit-target+)))
      "the constant must be Core's powLimit byte for byte")
  (let ((bl.store:*pow-limit-target*
          bl.store:+signet-pow-limit-target+))
    (is-true (bl.store:derive-target #x1e0377ae)
  ...
  (let ((bl.store:*pow-limit-target*
    ...
    (is-false (bl.store:derive-target #x1e0377ae)
...
reject and 2015 of every 2016 blocks failed :bad-difficulty."
  (let* ((bits #x1e0377ae)
         (prev-header (bl.ser:make-block-header
                       :version 4
