...
(defparameter +test-internal-reference-ceiling+ 3682
...
siblings) replaced the per-file reaches; 4,125 with the never-opened batch; 3,959 with the GA11 left-out net-peers and wallet batches; 3,957 with its rpc-config batch; 3,927 once thirty stale reaches named symbols their packages already exported; 3,902 when three parallel round-3 batches met at 3,932: two stale reaches, a shared reset fixture for a five-times-reached special, and the export of standard-output-script-p (a src caller, 26 test reaches) paid for the seam; 3,898 once the wallet batch's fee-rate suites named one %feerate-arg helper instead of fifteen reaches and its new wallet-RPC tests went through the exported dispatcher; 3,894 when %wt-at-height replaced six copies of the same chain-view setf in the wallet suite; 3,889 once wallet-data-directory and wallet-directory-of in tests/support/ replaced nine reaches into the wallet manager's paths; 3,865 after the round-3 init/RPC/HTTP batch, which added eleven white-box suites over internals that have no external caller and must not acquire one -- and paid for them several times over: %authorized-user folded forty-three CHECK-AUTH reaches in the RPC suite into one, gettxoutsetinfo and getmemoryinfo now go through the exported dispatcher (fourteen reaches gone), claim-directory and its three siblings in tests/support/ replaced thirteen directory-lock reaches across two files, %with-rpc-threads folded the worker-semaphore bindings, and six already-EXTERNAL names that were written with `::' now carry one colon; 3,849 when the package-path batch's two new reaches met main's lower ceiling and parse-json-rpc-request -- called by the HTTP server, reached twenty-five times by tests -- was exported and its reaches given one colon. 3,846 with the round-3 mempool/mining/RPC batch: its one new reach, the -blocknotify command a third notify test binds, paid for by four reaches at names their FOREIGN package already exports (hunchentoot's taskmaster accessors and raw-post-data slot, sb-format's %compiler-walk-format-string); ; 3,845 when the round-3 wallet batch drove its five new wallet-RPC tests through the exported dispatcher and folded five *wallet-rng* bindings into with-wallet-rng in tests/support/; . 3,835 with the round-3 storage/index/RPC batch, whose six new suites reach nothing new: they go through the exported dispatcher, and the one internal they read -- the digits a JSON number token carries -- is named once in tests/support/ as JSON-NUMBER-TOKEN, which BTC-AMOUNT is now built on. It paid for that by moving Core's InferDescriptor renderer down into BL.RPC, where the RPC layer that also wants it can call it: the four descriptor-suite reaches into the wallet's %infer-desc-body and %pairs-splitter now name BL.RPC:INFER-DESCRIPTOR-BODY and BL.RPC:DESCRIPTOR-PAIRS-SPLITTER with one colon, and two chain-control reaches went through DISPATCH-RPC-METHOD; 3,830 when the reindex and second wallet batches met at 3,839 and the RPC suite's nine getmempoolinfo calls went through DISPATCH-RPC-METHOD instead of the handler symbol; . 3,834 with the round-3 kv batch, whose ten new datadir-layout tests and two LevelDB log-line tests reach nothing internal at all -- they go through BL.STORE's exported resolvers and the CAPTURE-LOG-LINES fixture -- and which paid its one line by dropping the BL.TESTS:: prefix a storage test wrote for a helper defined in its own file; 3,813 when the round-5 net, storage and wallet batches met at 3,831: nine deriveaddresses reaches in the RPC suite became one %DERIVEADDRESSES helper, and the wallet batch drove its PSBT section's wallet RPCs through the exported dispatcher. 3,811 with the compat batch, whose Core-binary differential suite reaches nothing internal and whose one new fixture, RPC-RESULT-JSON in tests/support/, replaced the three copies of the server's encoder call in the RPC and wave-10 suites. 3,805 with the round-5 init batch, whose three new reaches (a zmq warm-up and two anchor loads) were paid by exporting the three ZMQ notifiers the zmq suite reached eleven times -- each has its src caller in the validation-interface hooks. 3,800 when that batch was rebased onto main at 3,811: four of its commits (fixed seeds, SetNetworkActive, the -connect notes, the refused dial) gave way to the net4 batch's implementations of the same behaviours, so their tests went with them. 3,791 when the p2p5 batch's BIP324 suite named V2-DETECT-INBOUND and V2-HANDSHAKE-OUTBOUND through one file-local helper each instead of thirteen reaches. 3,780 once the serve-requests suite named CACHED-GETADDR-RESPONSE through one helper instead of twelve reaches. 3,761 with the init4 batch: its anchors.dat tests replaced the old ANC1/ANC2 ones (thirty reaches into the private codec) with two tests over the file bytes, and the anchor, scheduler, addcon and index-thread entry points its new tests call -- each with its src caller in start-up or shutdown -- were exported and named with one colon. 3,694 with the blockindex batch: the block index moved into Core's blocks/index LevelDB, and the headerindex.dat snapshot and delta-log suites -- fifty-odd reaches into the retired writer, its CRC binding and its per-base-path record -- went with the format; the new suites reach only the change detector the old ones already did. 3,701 when that batch met the texts and wallet batches of the same round at main: their seven new reaches (VerifyDB's level ladder, the config-section warnings, the wallet-directory rules) are the round's price, paid against a ceiling sixty below the one they branched from. 3,688 with the cmpct batch, whose compact-block tests drive the blocktxn handler through one file-local %CBP-HANDLE-BLOCKTXN instead of four reaches. 3,682 when the same batch deleted DEFAULT-ASSUMEVALID, ASSUMEVALID-SKIP-HEIGHT and SCRIPT-SKIP-HEIGHT, which only their own test called, and that test's five reaches with them.
