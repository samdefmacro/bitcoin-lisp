...
(defparameter +test-internal-reference-ceiling+ 3900
...
siblings) replaced the per-file reaches; 4,125 with the never-opened batch; 3,959 with the GA11 left-out net-peers and wallet batches; 3,957 with its rpc-config batch; 3,927 once thirty stale reaches named symbols their packages already exported; 3,902 when three parallel round-3 batches met at 3,932: two stale reaches, a shared reset fixture for a five-times-reached special, and the export of standard-output-script-p (a src caller, 26 test reaches) paid for the seam; 3,900 once with-private-outbound-nonces in tests/support/ replaced the four loopback handshake tests' own reaches into the self-connection nonce registry.
