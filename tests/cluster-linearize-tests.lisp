...
(defun %dg-add (g fee size)
  (bl.mp:depgraph-add-transaction
   g (bl.mp:make-feefrac fee size)))

(defun %dg-anc (g i) (bl.mp:depgraph-ancestors g i))
(defun %dg-desc (g i) (bl.mp:depgraph-descendants g i))
  ...
  (let ((g (bl.mp:make-depgraph)))
    ...
    (loop for child from 1 below n
          do (let ((parents 0))
               ...
               (bl.mp:depgraph-add-dependencies g parents child)))
    g))
...
by ancestor count (Core cluster_linearize_tests.cpp:82-83). Assumes no holes."
  (let* ((n (bl.mp:depgraph-tx-count g))
         (v (make-array n)))
...
(defun %cl-chunks-equal-p (chunks expected)
  ...
  (and (= (length chunks) (length expected))
       (every (lambda (c e)
                (bl.mp:feefrac= c (bl.mp:make-feefrac
                                                  (first e) (second e))))
              chunks expected)))
            ...
            (loop
              (let ((old ancestors))
                (loop for j from 0 below 64 when (logbitp j ancestors)
                      do (setf ancestors
                               (logior ancestors
                                       (bl.mp:depgraph-reduced-parents g j))))
...
copy (cluster_linearize.h:135-148)."
  (let ((g (bl.mp:make-depgraph))
        (fr (bl.mp:make-feefrac 42 11)))
    (is (= 0 (bl.mp:depgraph-add-transaction g fr)))
    (is (= 1 (%dg-add g -13 7)))
    (is (= (%bits 0 1) (bl.mp:depgraph-positions g)))
    (is (= 2 (bl.mp:depgraph-position-range g)))
    (is (= 2 (bl.mp:depgraph-tx-count g)))
    ;; Mutating the caller's feefrac must not leak into the graph.
    (setf (bl.mp:feefrac-fee fr) 999)
    (is (= 42 (bl.mp:feefrac-fee
               (bl.mp:depgraph-tx-feerate g 0))))
    ;; New transactions are unconnected singletons.
    (is (= (%bits 0) (%dg-anc g 0)))
    (is (= (%bits 0) (%dg-desc g 0)))
    (is (bl.mp:feefrac=
         (bl.mp:make-feefrac 29 18)
         (bl.mp:depgraph-subset-feerate g (%bits 0 1))))))
...
(cluster_linearize_tests.cpp:152-166)."
  (let ((g (bl.mp:make-depgraph)))
    ...
    (bl.mp:depgraph-add-dependencies g (%bits 0 1) 2)
    (is (= (%bits 0) (%dg-anc g 0)))
    ...
    (is (= (%bits 0 1) (bl.mp:depgraph-reduced-parents g 2)))
    (is (= (%bits 2) (bl.mp:depgraph-reduced-children g 0)))
    (is (= (%bits 2) (bl.mp:depgraph-reduced-children g 1)))
    (is-true (bl.mp:depgraph-connected-p g))
    (is-true (bl.mp:depgraph-acyclic-p g))
    ;; {A,B} is NOT connected without C...
    (is-false (bl.mp:depgraph-connected-p g (%bits 0 1)))
    (is (= (%bits 0) (bl.mp:depgraph-find-connected-component g (%bits 0 1))))
    ;; ...and the empty set trivially is.
    (is-true (bl.mp:depgraph-connected-p g 0))
...
(cluster_linearize.h:179-200)."
  (let ((g (bl.mp:make-depgraph)))
    ...
    (bl.mp:depgraph-add-dependencies g (%bits 1) 2)
    (bl.mp:depgraph-add-dependencies g (%bits 0) 1)
    (is (= (%bits 0 1 2) (%dg-anc g 2)))
    (is (= (%bits 0 1 2) (%dg-desc g 0)))
    ;; Reduced parents keep only the direct edge.
    (is (= (%bits 1) (bl.mp:depgraph-reduced-parents g 2)))
    (is (= (%bits 1) (bl.mp:depgraph-reduced-children g 0)))
    ;; Re-adding an implied dependency (C on A) is a no-op.
    (bl.mp:depgraph-add-dependencies g (%bits 0) 2)
    (is (= (%bits 1) (bl.mp:depgraph-reduced-parents g 2)))
    (is-true (%cl-depgraph-sane-p g))))
...
removed middleman keeps grandparents as ancestors (cluster_linearize.h:150-173)."
  (let ((g (bl.mp:make-depgraph)))
    ...
    (bl.mp:depgraph-remove-transactions g (%bits 1))
    (is (= (%bits 0 2) (bl.mp:depgraph-positions g)))
    (is (= 3 (bl.mp:depgraph-position-range g)))
    (is (= (%bits 0 2) (%dg-anc g 2)))
    (is (= (%bits 0 2) (%dg-desc g 0)))
    (is (= (%bits 0) (bl.mp:depgraph-reduced-parents g 2)))
    ;; Still connected through the whole-graph relation, even with the parent
    ;; missing from the subset (cluster_linearize.h:255-262).
    (is-true (bl.mp:depgraph-connected-p g (%bits 0 2)))
    (is (= (%bits 0 2) (bl.mp:depgraph-connected-component g (%bits 0 2) 0)))
    ;; Removing the tail entry shrinks the position range...
    (bl.mp:depgraph-remove-transactions g (%bits 2))
    (is (= (%bits 0) (bl.mp:depgraph-positions g)))
    (is (= 1 (bl.mp:depgraph-position-range g)))
...
D(1,3) E(1,1); deps C->A, D->A, D->B, E->D (cluster_linearize_tests.cpp:189-192)."
  (let ((g (bl.mp:make-depgraph)))
    ...
    (bl.mp:depgraph-add-dependencies g (%bits 0) 2)
    (bl.mp:depgraph-add-dependencies g (%bits 0 1) 3)
    (bl.mp:depgraph-add-dependencies g (%bits 3) 4)
    (is (= (%bits 0) (%dg-anc g 0)))
    (is (= (%bits 1) (%dg-anc g 1)))
    (is (= (%bits 0 2) (%dg-anc g 2)))
    ...
    (is (= (%bits 0 1) (bl.mp:depgraph-reduced-parents g 3)))
    (is (= (%bits 3) (bl.mp:depgraph-reduced-parents g 4)))
    (is (= (%bits 2 3) (bl.mp:depgraph-reduced-children g 0)))
    (is (= (%bits 3) (bl.mp:depgraph-reduced-children g 1)))
    (is-true (bl.mp:depgraph-connected-p g))
    (is-true (bl.mp:depgraph-acyclic-p g))
    ;; Connected components within {C,D,E}: {C} and {D,E}.
    (is (= (%bits 2) (bl.mp:depgraph-connected-component g (%bits 2 3 4) 2)))
    (is (= (%bits 3 4) (bl.mp:depgraph-connected-component g (%bits 2 3 4) 3)))
    (is-false (bl.mp:depgraph-connected-p g (%bits 2 3 4)))
    ;; Topological subsets: closed under ancestors.
    (is-true (bl.mp:topological-subset-p g (%bits 0)))
    (is-true (bl.mp:topological-subset-p g (%bits 0 2)))
    (is-false (bl.mp:topological-subset-p g (%bits 2)))
    (is-true (bl.mp:topological-subset-p g (%bits 0 1 3)))
    (is-false (bl.mp:topological-subset-p g (%bits 3 4)))
    (is-true (bl.mp:topological-subset-p g (%bits 0 1 2 3 4)))
    ;; Topological order and the linearization validity check.
    (is (equal '(0 1 2 3 4) (bl.mp:depgraph-topo-sorted g)))
...
(test chunk-linearization-basics
  ...
  (let ((g (bl.mp:make-depgraph)))
    ...
    (let ((info (bl.mp:chunk-linearization-info g #(0 1 2))))
      (is (= 2 (length info)))
      (is (= (%bits 0 1) (bl.mp:setinfo-transactions (first info))))
      (is (= (%bits 2) (bl.mp:setinfo-transactions (second info))))
      (is (bl.mp:feefrac=
           (bl.mp:make-feefrac 6 2)
           (bl.mp:setinfo-feerate (first info)))))))
...
(test ancestor-sort-linearization-cpfp
  ...
  (let ((g (bl.mp:make-depgraph)))
    ...
    (bl.mp:depgraph-add-dependencies g (%bits 0) 1)
    (is (equalp #(0 1 2) (bl.mp:ancestor-sort-linearization g))))
  ;; With a better standalone (0.9), it goes first.
  (let ((g (bl.mp:make-depgraph)))
    (%dg-add g 1 100) (%dg-add g 99 100) (%dg-add g 90 100)
    (bl.mp:depgraph-add-dependencies g (%bits 0) 1)
    (is (equalp #(2 0 1) (bl.mp:ancestor-sort-linearization g)))))
  ...
  (let ((rng (%cl-make-rng 88172645463325252)))
    (dotimes (iter 200)
      (let* ((n (1+ (funcall rng 12)))
             ...
             (lin0 (bl.mp:ancestor-sort-linearization g))
             (lin1 (bl.mp:post-linearize g lin0)))
        (is-true (%cl-depgraph-sane-p g))
        ...
        (is (member (bl.mp:compare-chunks
        ...
        (let* ((lin2 (%cl-shuffled-topo-linearization rng g))
               (lin3 (bl.mp:post-linearize g lin2)))
