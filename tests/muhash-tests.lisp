...
(defun %mh-from-int (i)
  ...
  (let ((tmp (make-array 32 :element-type '(unsigned-byte 8) :initial-element 0)))
    (setf (aref tmp 0) i)
    (bitcoin-lisp.crypto:make-muhash tmp)))
...
internal (SHA256) byte order like Core's uint256 storage."
  (bitcoin-lisp.crypto:bytes-to-hex
   (bitcoin-lisp.crypto:reverse-bytes (bitcoin-lisp.crypto:muhash-finalize mu))))
...
same published hash."
  (let ((expected "10d312b100cbd32ada024a6646e40d3482fcff103668d2625f10002a607d5863"))
    ...
    (let ((acc (%mh-from-int 0)))
      (bitcoin-lisp.crypto:muhash-combine acc (%mh-from-int 1))
      (bitcoin-lisp.crypto:muhash-divide acc (%mh-from-int 2))
    ...
    (let ((acc2 (%mh-from-int 0))
      ...
      (bitcoin-lisp.crypto:muhash-insert acc2 tmp1)
      (bitcoin-lisp.crypto:muhash-remove acc2 tmp2)
...
finalizes to the same hash."
  (let* ((table #(3 6 1 5))               ; some inserts (bit2=0) and removes (bit2=1)
         (baseline nil))
    (dotimes (order 4)
      (let ((acc (bitcoin-lisp.crypto:make-muhash)))
        (dotimes (i 4)
          (let ((tv (aref table (logxor i order))))
            (if (logtest tv 4)
                (bitcoin-lisp.crypto:muhash-divide acc (%mh-from-int (logand tv 3)))
                (bitcoin-lisp.crypto:muhash-combine acc (%mh-from-int (logand tv 3))))))
        (let ((h (%mh-hex acc)))
...
denominator exactly cancels the numerator)."
  (let ((x (%mh-from-int 9))
        (y (%mh-from-int 5))
        (z (bitcoin-lisp.crypto:make-muhash)))
    (bitcoin-lisp.crypto:muhash-combine z x)       ; z = X
    (bitcoin-lisp.crypto:muhash-combine z y)       ; z = X*Y
    (bitcoin-lisp.crypto:muhash-combine y x)       ; y = Y*X
    (bitcoin-lisp.crypto:muhash-divide z y)        ; z = 1
    (is (string= (%mh-hex (bitcoin-lisp.crypto:make-muhash)) (%mh-hex z)))))
...
interleaving."
  (let ((acc (bitcoin-lisp.crypto:make-muhash))
    ...
    (bitcoin-lisp.crypto:muhash-insert acc a)
    (let ((before (%mh-hex acc)))
      (bitcoin-lisp.crypto:muhash-insert acc b)
      (bitcoin-lisp.crypto:muhash-remove acc b)
...
txid=0xAB*32, vout=1, height=100, coinbase, amount=5e9, script=OP_TRUE."
  (let* ((txid (make-array 32 :element-type '(unsigned-byte 8) :initial-element #xAB))
         ...
         (elem (coerce (bitcoin-lisp.storage:coin-muhash-element
    ...
    (is (string= "abababababababababababababababababababababababababababababababab01000000c900000000f2052a010000000151"
                 (bitcoin-lisp.crypto:bytes-to-hex elem)))
    (let ((mu (bitcoin-lisp.crypto:make-muhash)))
      (bitcoin-lisp.crypto:muhash-insert mu elem)
...
= 1, so value = 1, SHA256 of the 384-byte LE encoding of 1)."
  (let* ((one (bitcoin-lisp.crypto::%le-integer-to-bytes 1 384))
         (expected (bitcoin-lisp.crypto:bytes-to-hex
                    (bitcoin-lisp.crypto:reverse-bytes (bitcoin-lisp.crypto:sha256 one)))))
    (is (string= expected (%mh-hex (bitcoin-lisp.crypto:make-muhash))))))
