...
;;; Skip-list ancestor tests
;;;
;;; Mirrors Bitcoin Core's GetSkipHeight / GetAncestor / BuildSkip
;;; (refs/bitcoin/src/chain.cpp). The skip pointer lets get-ancestor
;;; reach any height in O(log depth) instead of walking prev-entry.

(test get-skip-height-matches-bitcoin-core
  "GetSkipHeight: 0 for heights <2, InvertLowestOne(height) for even,
InvertLowestOne(InvertLowestOne(h-1)) + 1 for odd. Spot-check against
the formula in chain.cpp:73-81 for a range that exercises both even
and odd cases."
  ;; height < 2 → 0
  (is (= 0 (bitcoin-lisp.storage:get-skip-height 0)))
  (is (= 0 (bitcoin-lisp.storage:get-skip-height 1)))
  ;; even: InvertLowestOne(h) = h & (h-1)
  (is (= 0 (bitcoin-lisp.storage:get-skip-height 2)))
  (is (= 0 (bitcoin-lisp.storage:get-skip-height 4)))
  (is (= 4 (bitcoin-lisp.storage:get-skip-height 6)))
  (is (= 0 (bitcoin-lisp.storage:get-skip-height 8)))
  (is (= 8 (bitcoin-lisp.storage:get-skip-height 10)))
  (is (= 12 (bitcoin-lisp.storage:get-skip-height 14)))
  ;; odd: InvertLowestOne(InvertLowestOne(h-1)) + 1
  (is (= 1 (bitcoin-lisp.storage:get-skip-height 3)))
  (is (= 1 (bitcoin-lisp.storage:get-skip-height 5)))
  (is (= 1 (bitcoin-lisp.storage:get-skip-height 7)))
  (is (= 9 (bitcoin-lisp.storage:get-skip-height 15)))
  ;; Sanity: skip height is always strictly less than the input (per the
  ;; precondition GetAncestor relies on).
  (loop for h from 2 to 1000
        do (is (< (bitcoin-lisp.storage:get-skip-height h) h))))

(test get-ancestor-returns-correct-entry
  "Build a chain of 1000 blocks, ask for the entry at every multiple
of 17, verify height matches and the skip walk lands on the same
entry as walking prev-entry linearly. The 17-step stride pokes both
even and odd height cases."
  (let* ((state (bitcoin-lisp.storage:make-chain-state))
         (heights (loop for h from 0 to 1000 collect h))
         (tip (build-chain-entries heights :chain-state state)))
    (loop for target from 0 to 1000 by 17
          do (let ((skip-result
                     (bitcoin-lisp.storage:get-ancestor tip target))
                   (linear-result
                     (let ((e tip))
                       ...
                       e)))
               (is (not (null skip-result)))
               (is (= target (bitcoin-lisp.storage:block-index-entry-height skip-result)))
               (is (eq skip-result linear-result))))))

(test get-ancestor-rejects-out-of-range
  "Above-tip or negative heights return NIL — Bitcoin Core's parity
guard (chain.cpp:85)."
  (let* ((state (bitcoin-lisp.storage:make-chain-state))
         (tip (build-chain-entries '(0 1 2 3 4 5) :chain-state state)))
    (is (null (bitcoin-lisp.storage:get-ancestor tip 6)))
    (is (null (bitcoin-lisp.storage:get-ancestor tip 100)))
    (is (eq tip (bitcoin-lisp.storage:get-ancestor tip 5)))))

(test build-skip-pointer-sets-correct-target
  "After build-skip-pointer, skip-entry points to the ancestor at
GetSkipHeight(height). The link-header-entries path builds these in
height order so each lookup finds the target already populated."
  (let* ((state (bitcoin-lisp.storage:make-chain-state))
         (tip (build-chain-entries (loop for h from 0 to 20 collect h)
                                   :chain-state state)))
    (loop for cursor = tip then (bitcoin-lisp.storage:block-index-entry-prev-entry cursor)
          while cursor
          do (let* ((h (bitcoin-lisp.storage:block-index-entry-height cursor))
                    (expected-skip-height (bitcoin-lisp.storage:get-skip-height h))
                    (skip (bitcoin-lisp.storage:block-index-entry-skip-entry cursor)))
               (cond
                 ;; Genesis (height 0) has no prev and so no skip.
                 ((zerop h) (is (null skip)))
                 (t (is (not (null skip)))
                    (is (= expected-skip-height
                           (bitcoin-lisp.storage:block-index-entry-height skip)))))))))
