...
BL:*NODE* is cleared BEFORE the body, not only restored after. START-NODE
opens with \"(when *node* (log-warn \"Node already running, stopping first\")
(stop-node))\", so a global node another suite left behind is torn down by
this test's first start -- and STOP-NODE waits up to 600 SECONDS for that
node's sync thread. That is longer than any test runner's budget, so the run
does not fail, it stops: the battery ends inside the test with no failure and
no check count. Nothing here should ever inherit another suite's node.

The other thing that matters is SB-EXT:*INVOKE-DEBUGGER-HOOK*: START-NODE installs a
...
would stop the next run before it began."
  (let ((hook (gensym "HOOK")) (node (gensym "NODE"))
    ...
    `(let ((,base-var (ensure-directories-exist
           ...
           (,network bl:*network*))
       ;; Both cleared, not merely restored: START-NODE would tear down a node
       ;; another suite left in BL:*NODE* (see below), and the block-index
       ;; persist hook it installs reads BL:*NODE* when a coins-cache sync
       ;; calls it -- after the node is gone that is a chain-state with no base
       ;; path, and save-header-index type-errors in merge-pathnames.
       (setf bl:*node* nil
