...
(defun %ib-p2pkh-spk ()
  ...
  (%ib-bytes #x76 #xa9 #x14
             (bitcoin-lisp.crypto:hash160
              (bitcoin-lisp.crypto:derive-public-key +ib-privkey+))
             #x88 #xac))

(defun %ib-p2tr-spk ()
  "P2TR scriptPubKey (key-path only) for +IB-PRIVKEY+."
  (%ib-bytes #x51 #x20
             (bitcoin-lisp.crypto:derive-xonly-pubkey
              (bitcoin-lisp.crypto:taproot-tweak-private-key +ib-privkey+))))
...
encoding stays canonical and low-S, so only the ECDSA check can reject it."
  (let* ((sighash (bitcoin-lisp.coalton.interop::compute-legacy-sighash
         ...
         (sig (%ib-bytes (bitcoin-lisp.crypto:sign-ecdsa +ib-privkey+ sighash) 1)))
    (when corrupt
      (setf (aref sig 10) (logxor (aref sig 10) #xff)))
    (%ib-bytes (%ib-push sig)
               (%ib-push (bitcoin-lisp.crypto:derive-public-key +ib-privkey+)))))
...
their amounts and scriptPubKeys, so it is exactly what a chained spend needs."
  (let* ((bitcoin-lisp.coalton.interop::*current-tx* tx)
         ...
         (sighash (bitcoin-lisp.coalton.interop::compute-bip341-sighash
                   (bitcoin-lisp.storage:utxo-entry-value (aref spent-utxos input-index))
                   0 nil nil))
         (sig (bitcoin-lisp.crypto:sign-schnorr
               (bitcoin-lisp.crypto:taproot-tweak-private-key +ib-privkey+)
               sighash)))
    (when corrupt
      (setf (aref sig 10) (logxor (aref sig 10) #xff)))
...
(defun %ib-entry (value script-pubkey height)
  ...
  (bitcoin-lisp.storage::make-utxo-entry :value value
...
(values chain-state utxo-set prev-hash bits now subsidy)."
  (let* ((node (%regtest-node-fixture suffix))
    ...
    (setf (bitcoin-lisp::node-utxo-set node) utxo)
    (bitcoin-lisp.storage:add-utxo utxo (%ib-coin) 0 +ib-coin-value+
                                   (%p2sh-optrue-spk) 0 :coinbase nil)
    (values cs utxo
            (bitcoin-lisp.storage:best-block-hash cs)
            (bitcoin-lisp.serialization:block-header-bits
             (bitcoin-lisp::make-genesis-header :regtest))
            (bitcoin-lisp.serialization:get-unix-time)
            (bitcoin-lisp.validation::calculate-block-subsidy 1))))
...
commitment over TXS) followed by TXS."
  (let* ((wtxids (cons (make-array 32 :element-type '(unsigned-byte 8)
                                      :initial-element 0)
                       (mapcar #'bitcoin-lisp.serialization:transaction-wtxid txs)))
         (combined (make-array 64 :element-type '(unsigned-byte 8) :initial-element 0)))
    (replace combined (bitcoin-lisp.validation:compute-merkle-root wtxids) :start1 0)
    (let* ((coinbase (bitcoin-lisp.mining:build-coinbase-transaction
                      1 cb-value
           ...
           (blk (bitcoin-lisp.serialization:make-bitcoin-block
                 :header (bitcoin-lisp.serialization:make-block-header
                          ...
                          :merkle-root (bitcoin-lisp.validation:compute-merkle-root
                                        (mapcar #'bitcoin-lisp.serialization:transaction-hash
                                                all))
                          :timestamp timestamp
                          :bits bits
                          :nonce 0)
                 :transactions all)))
      (or (bitcoin-lisp.mining:mine-block blk)
...
(defun %ib-validate (blk cs utxo now)
  ...
  (multiple-value-bind (valid error fees)
      (bitcoin-lisp.validation:validate-block blk cs utxo 1 now)
    (values valid error
            (and fees (bitcoin-lisp.coalton.interop:unwrap-satoshi fees)))))
  ...
  (%with-regtest
   (multiple-value-bind (cs utxo prev bits now subsidy) (%ib-env "ib-badsig")
     (let* ((p2pkh (%ib-p2pkh-spk))
            ...
            (a-txid (bitcoin-lisp.serialization:transaction-hash tx-a))
       ...
       (bitcoin-lisp.storage:add-utxo utxo confirmed 0 90000000 p2pkh 0 :coinbase nil)
  ...
  (%with-regtest
   (multiple-value-bind (cs utxo prev bits now subsidy) (%ib-env "ib-mixed")
     (let ((p2pkh (%ib-p2pkh-spk))
       ...
       (bitcoin-lisp.storage:add-utxo utxo confirmed 0 50000000 p2pkh 0 :coinbase nil)
   ...
   (multiple-value-bind (cs utxo prev bits now subsidy) (%ib-env "ib-taproot")
     (let* ((p2tr (%ib-p2tr-spk))
       ...
       (let ((extra (make-hash-table :test 'equalp))
             (inputs (bitcoin-lisp.serialization:transaction-inputs unsigned)))
         (setf (gethash (cons a-txid 0) extra) (aref spent 0))
         (is (null (bitcoin-lisp.validation::collect-spent-utxos inputs utxo)))
         (let ((collected (bitcoin-lisp.validation::collect-spent-utxos inputs utxo extra)))
           (is (equalp spent collected))
           (is (not (null (bitcoin-lisp.coalton.interop::precomputed-sighash-data-sha-amounts
                           (bitcoin-lisp.coalton.interop:init-precomputed-sighash
                            unsigned collected)))))))))))
  ...
  (%with-regtest
   (multiple-value-bind (cs utxo prev bits now subsidy) (%ib-env "ib-dspend")
     (let* ((tx-x (%pkg-tx (%ib-coin) 0 90000000))
       ...
       (is (not (equalp (bitcoin-lisp.serialization:transaction-hash tx-x)
                        (bitcoin-lisp.serialization:transaction-hash tx-y))))
