...
can read the spending transaction back and confirm it."
  (let ((dir (%tsi-tmpdir "find")))
    (unwind-protect
         (let ((idx (bitcoin-lisp.storage:init-txospender-index dir)))
           (unwind-protect
                (multiple-value-bind (block spender) (%tsi-spending-block
                  ...
                  (let ((hash (bitcoin-lisp.serialization:block-header-hash
                    ...
                    (is (= 2 (bitcoin-lisp.storage:txospenderindex-add-block idx block hash)))
                    (let ((locs (bitcoin-lisp.storage:txospenderindex-locators
                                 idx (bitcoin-lisp.serialization:outpoint-hash
                                      (%tsi-outpoint #xA1 0))
                                 0)))
                      (is (= 1 (length locs)))
                    ...
                    (is (null (bitcoin-lisp.storage:txospenderindex-locators
                               idx (bitcoin-lisp.serialization:outpoint-hash
                                    (%tsi-outpoint #xA1 0))
                               1)))
                    ...
                    (is (null (bitcoin-lisp.storage:txospenderindex-locators
                               idx (bitcoin-lisp.serialization:outpoint-hash
                                    (%tsi-outpoint #xFF 0))
                               0)))))
             (bitcoin-lisp.storage:close-txospender-index idx)))
           ...
           (unwind-protect
                (let* ((block (%tsi-spending-block (list (%tsi-outpoint #xB1 0))))
                       (hash (bitcoin-lisp.serialization:block-header-hash
                              (bitcoin-lisp.serialization:bitcoin-block-header block)))
                       (txid (bitcoin-lisp.serialization:outpoint-hash (%tsi-outpoint #xB1 0))))
                  (bitcoin-lisp.storage:txospenderindex-add-block idx block hash)
                  (is (= 1 (length (bitcoin-lisp.storage:txospenderindex-locators idx txid 0))))
                  (is (= 1 (bitcoin-lisp.storage:txospenderindex-remove-block idx block hash)))
                  (is (null (bitcoin-lisp.storage:txospenderindex-locators idx txid 0))
                      "a disconnected block left its spender entries behind")
                  ;; Idempotent: erasing twice is not an error and does not
                  ;; resurrect anything.
                  (bitcoin-lisp.storage:txospenderindex-remove-block idx block hash)
                  (is (null (bitcoin-lisp.storage:txospenderindex-locators idx txid 0))))
             (bitcoin-lisp.storage:close-txospender-index idx)))
      (ignore-errors (uiop:delete-directory-tree dir :validate t :if-does-not-exist :ignore)))))
...
reason (index/txospenderindex.cpp:66-70)."
  (let ((dir (%tsi-tmpdir "salt")))
    (unwind-protect
         (let (k0 k1)
           (let ((idx (bitcoin-lisp.storage:init-txospender-index dir)))
             (setf k0 (bitcoin-lisp.storage::txospender-index-k0 idx)
                   k1 (bitcoin-lisp.storage::txospender-index-k1 idx))
           ...
           (let ((idx (bitcoin-lisp.storage:init-txospender-index dir)))
             (unwind-protect
                  (progn
                    (is (= k0 (bitcoin-lisp.storage::txospender-index-k0 idx)))
                    (is (= k1 (bitcoin-lisp.storage::txospender-index-k1 idx)))
                    ;; And the record written before the reopen is still findable.
                    (is (= 1 (length (bitcoin-lisp.storage:txospenderindex-locators
                                      idx
         ...
         (let ((idx (bitcoin-lisp.storage:init-txospender-index dir)))
           (unwind-protect
                (let ((hash (make-array 32 :element-type '(unsigned-byte 8)
                                           :initial-element #xD1)))
                  (is (= -1 (bitcoin-lisp.storage:txospenderindex-height idx)))
                  (bitcoin-lisp.storage:txospenderindex-set-best-block idx hash 12345)
                  (multiple-value-bind (h height)
                      (bitcoin-lisp.storage:txospenderindex-best-block idx)
                    (is (equalp hash h))
                    (is (= 12345 height)))
                  (is (= 12345 (bitcoin-lisp.storage:txospenderindex-height idx))))
             (bitcoin-lisp.storage:close-txospender-index idx)))
      (ignore-errors (uiop:delete-directory-tree dir :validate t :if-does-not-exist :ignore)))))
...
does not have to test for it — the same contract the txindex has."
  (let ((idx (bitcoin-lisp.storage:init-txospender-index
              (uiop:temporary-directory) :enabled nil)))
    (multiple-value-bind (block) (%tsi-spending-block (list (%tsi-outpoint #xE1 0)))
      (let ((hash (bitcoin-lisp.serialization:block-header-hash
                   (bitcoin-lisp.serialization:bitcoin-block-header block))))
        (is (= 0 (bitcoin-lisp.storage:txospenderindex-add-block idx block hash)))
        (is (= 0 (bitcoin-lisp.storage:txospenderindex-remove-block idx block hash)))
        (is (null (bitcoin-lisp.storage:txospenderindex-locators
                   idx (bitcoin-lisp.serialization:outpoint-hash (%tsi-outpoint #xE1 0)) 0)))
        (is (null (bitcoin-lisp.storage:txospenderindex-best-block idx)))
        (is (= -1 (bitcoin-lisp.storage:txospenderindex-height idx)))))))
