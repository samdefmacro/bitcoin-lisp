  ...
  (dolist (ntx '(1 2 3 4 5 7 8 16))
    (let* ((txids (%proof-hashes ntx))
      ...
      (multiple-value-bind (bits hashes)
          (bl.net:build-partial-merkle-tree txid-vec match)
  ...
  (let* ((ntx 6)
    ...
    (multiple-value-bind (bits hashes)
      ...
      (let* ((header (make-array 80 :element-type '(unsigned-byte 8) :initial-element 7))
             (bytes (bl.net:serialize-merkle-block header ntx hashes bits)))
        (multiple-value-bind (h2 ntx2 hashes2 bits2)
            (bl.net:parse-merkle-block bytes)
          (is (equalp header h2))
          (is (= ntx ntx2))
          ...
          (is (equalp (bl.net:extract-partial-merkle-tree ntx bits hashes)
                      (bl.net:extract-partial-merkle-tree ntx2 bits2 hashes2))))))))
  ...
  (let* ((ntx 8)
    ...
    (multiple-value-bind (bits hashes)
      ...
      (let ((tampered (mapcar #'copy-seq hashes)))
        ...
        (is (not (equalp root (bl.net:extract-partial-merkle-tree
                               ntx bits tampered))))))))
  ...
  (with-temp-directory (dir "bl-initload")
    (let ((node (make-test-node)))
      ...
      (flet ((loaded ()
        ...
        (let ((bl.mp:*mempool-load-tried* nil)
              (during :not-called)
              (import-step 'bl::%import-external-block-files)
              (saved-import nil))
          (unwind-protect
               (progn
                 (setf saved-import (fdefinition import-step)
                       (fdefinition import-step)
                       (lambda (node paths)
                         (declare (ignore node paths))
                         (setf during (loaded))))
                 (bl::%initload node (list "/nonexistent/bootstrap.dat")))
            (when saved-import (setf (fdefinition import-step) saved-import)))
...
anything — and that the count comparison rejects it."
  (let* ((txids (%proof-hashes 4))
    ...
    (multiple-value-bind (forged-root forged-matched)
        ...
        (bl.net:extract-partial-merkle-tree 2 (list t t t) (list a b))
