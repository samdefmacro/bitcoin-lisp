            ...
            (when (char= sep #\Space)   ; final line of this message
              (if (>= code 600)
                  (progn                ; async notification: drop, keep reading
                    (bitcoin-lisp:log-cat "tor" "Ignoring async event ~D" code)
                    (setf code 0 lines '()))
...
connection. SENSITIVE elides the command from the debug log (passwords)."
  (bitcoin-lisp:log-cat "tor" "-> ~A" (if sensitive "AUTHENTICATE <elided>" cmd))
...
itself guards key material. Returns T on success."
  (handler-case
      (progn
        ...
        t)
    (error (e)
      (bitcoin-lisp:log-warn "tor: Error writing service private key to ~A: ~A"
                             path e)
      nil)))
    ...
    (let ((client-nonce (ironclad:random-data +tor-nonce-size+)))
      (multiple-value-bind (code lines)
          (tor-command ctl (format nil "AUTHCHALLENGE SAFECOOKIE ~A"
                                   (bitcoin-lisp.crypto:bytes-to-hex client-nonce)))
        (unless (eql code +tor-reply-ok+)
          (bitcoin-lisp:log-warn "tor: SAFECOOKIE authentication challenge failed")
          (return-from %tor-authenticate-safecookie nil))
        (multiple-value-bind (type args) (split-tor-reply-line (first lines))
          (unless (string= type "AUTHCHALLENGE")
            (bitcoin-lisp:log-warn "tor: Invalid reply to AUTHCHALLENGE")
            (return-from %tor-authenticate-safecookie nil))
          (let ((mapping (parse-tor-reply-mapping args)))
            (unless mapping
              (bitcoin-lisp:log-warn "tor: Error parsing AUTHCHALLENGE parameters: ~A" args)
              (return-from %tor-authenticate-safecookie nil))
            (let ((server-hash (bitcoin-lisp.crypto:hex-to-bytes
              ...
              (unless (equalp (compute-safecookie-response
                ...
                (bitcoin-lisp:log-warn
                 ...
                 (bitcoin-lisp.crypto:bytes-to-hex server-hash))
                (return-from %tor-authenticate-safecookie nil))
              (tor-command
...
SAFECOOKIE. Returns T on a 250 to AUTHENTICATE."
  (multiple-value-bind (code lines) (tor-command ctl "PROTOCOLINFO 1")
    (unless (eql code +tor-reply-ok+)
      (bitcoin-lisp:log-warn "tor: Requesting protocol info failed")
          ...
          (cond
            ...
            ((string= type "VERSION")
             (let ((v (tor-mapping-value (parse-tor-reply-mapping args) "Tor")))
               (when v
                 (bitcoin-lisp:log-cat "tor" "Connected to Tor version ~A" v)))))))
             ...
             (auth-code
               (cond
                 ((and password (plusp (length password)))
                  (cond
                    ((member "HASHEDPASSWORD" methods :test #'string=)
                     (bitcoin-lisp:log-cat "tor" "Using HASHEDPASSWORD authentication")
                 ...
                 ((member "NULL" methods :test #'string=)
                  (bitcoin-lisp:log-cat "tor" "Using NULL authentication")
                 ...
                 ((member "HASHEDPASSWORD" methods :test #'string=)
                  ...
                  nil)
                 (t
                  (bitcoin-lisp:log-warn "tor: No supported authentication method")
                  nil))))
        (cond
          ((eql auth-code +tor-reply-ok+)
           (bitcoin-lisp:log-cat "tor" "Authentication successful")
           t)
          (auth-code
           (bitcoin-lisp:log-warn "tor: Authentication failed")
           nil)
          (t nil))))))
    ...
    (let ((socks-location nil))
      (cond
        ((eql code +tor-reply-ok+)
         ...
         (if socks-location
             (bitcoin-lisp:log-cat "tor" "Get SOCKS port command yielded ~A"
                                   socks-location)
             (bitcoin-lisp:log-warn "tor: Get SOCKS port command returned nothing")))
        ...
        (t
         (bitcoin-lisp:log-warn "tor: Get SOCKS port command failed; error code ~D"
                                code)))
      (multiple-value-bind (host port)
        ...
        (bitcoin-lisp:log-cat "tor" "Configuring onion proxy for ~A:~D" host port)
    ...
    (cond
      ((eql code +tor-reply-ok+)
       (let ((sid nil))
         ...
         (unless sid
           (bitcoin-lisp:log-warn "tor: Error parsing ADD_ONION parameters:~{ ~A~}"
                                  lines)
           (return-from %tor-add-onion nil))
         (let ((pubkey (parse-onion-address (concatenate 'string sid ".onion"))))
           (unless pubkey
             (bitcoin-lisp:log-warn "tor: Invalid ServiceID in ADD_ONION reply: ~A" sid)
             (return-from %tor-add-onion nil))
           ...
           (when (and (tor-controller-private-key ctl)
             ...
             (bitcoin-lisp:log-cat "tor" "Cached service private key to ~A"
                                   (tor-controller-private-key-file ctl)))
      ...
      ((eql code +tor-reply-unrecognized+)
       ...
       nil)
      (code
       (bitcoin-lisp:log-warn "tor: Add onion failed; error code ~D" code)
       nil)
      (t nil))))
      ...
      (progn
        (setf (tor-controller-socket ctl)
              (usocket:socket-connect (tor-controller-control-host ctl)
                                      ...
                                      :timeout 10))
        (bitcoin-lisp:log-cat "tor" "Successfully connected to Tor control port ~A:~D"
               ...
               (progn
                 ...
                 (unwind-protect
                      (handler-case (%tor-session ctl)
                        (tor-control-error (e)
                          (bitcoin-lisp:log-warn "tor: ~A"
                                                 (tor-control-error-message e)))
                        (error (e)
                          (bitcoin-lisp:log-warn "tor: control session error: ~A" e)))
                   (%tor-disconnect ctl)
  ...
  (multiple-value-bind (host port) (parse-torcontrol-spec control-spec)
    (let* ((key-file (merge-pathnames +onion-private-key-file+ data-directory))
      ...
      (when (tor-controller-private-key ctl)
        (bitcoin-lisp:log-cat "tor" "Reading cached private key from ~A" key-file))
