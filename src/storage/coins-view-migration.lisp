...
it (CRC mismatch, version mismatch, truncated file)."
  ...
  (let ((utxo-set (make-utxo-set)))
    ...
    (let* ((total (hash-table-count (utxo-set-entries utxo-set)))
           ...
           (in-batch 0)
           (batch nil))
      (bitcoin-lisp:log-info "Migrating ~D UTXO entries from ~A → ~A"
        ...
        (flet ((open-batch () (setf batch (leveldb-make-writebatch)))
               (commit-batch ()
                 (when batch
                   ...
                   (setf batch nil
                         in-batch 0)
                   (bitcoin-lisp:log-info "Migration progress: ~D / ~D" written total))))
          (open-batch)
          (unwind-protect
               (progn
      ...
      (bitcoin-lisp:log-info "Migration complete: ~D entries written" written)
      written)))
