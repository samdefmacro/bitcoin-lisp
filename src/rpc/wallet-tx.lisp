...
(defun %wallet-parse-txid (value)
  (unless (and (stringp value) (valid-hex-hash-p value))
  ...
  (let ((confirms (wallet-tx-depth wallet wtx)))
    `(("confirmations" . ,confirms)
      ...
      ,@(if (eq (wallet-tx-state wtx) :confirmed)
            `(("blockhash" . ,(hash-to-hex (wallet-tx-block-hash wtx)))
      ...
      ("walletconflicts" . ,(or (mapcar #'hash-to-hex
                                ...
                                #()))
      ("mempoolconflicts" . ,(or (mapcar #'hash-to-hex
...
(label count skip include_watchonly)."
  (let ((wallet (wallet-for-request node))
    ...
    (unless (and (integerp count) (>= count 0))
      (error 'rpc-error :code +rpc-invalid-parameter+ :message "Negative count"))
    (unless (and (integerp skip) (>= skip 0))
      (error 'rpc-error :code +rpc-invalid-parameter+ :message "Negative from"))
    (with-node-lock (node)
      (with-wallet-lock (wallet)
        (let ((ret '())
              (ordered (wallet-tx-ordered wallet)))
