...
Returns the number of new peers connected."
  ...
  ;; setnetworkactive dropped, even while networking stays disabled. Every
  ;; retirement path (disconnect-peer, record-misbehavior, setban) ends in
  ;; :disconnected, so this one state is the whole reap.
  (bt:with-recursive-lock-held ((node-lock node))
    (setf (node-peers node)
          (remove-if (lambda (p) (eq (bl.net:peer-state p) :disconnected))
                     (node-peers node))))
...
block that arrives mid-pass -- was connected and never announced, and a
reorg announced nothing at all. In the 2026-09-13 sweep example_test.py's
