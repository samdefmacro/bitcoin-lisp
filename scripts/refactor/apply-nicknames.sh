...
# nicknames. The rules are derived from bitcoin-lisp::*package-nicknames* in
# src/package.lisp, the one table. Idempotent: run it on any branch after a
...
# excluded: its baselines are real package names as data. The
# install-package-nicknames call after each defpackage is read in CL-USER,
# where no nickname exists, so it keeps its full name.
set -euo pipefail
cd "$(dirname "$0")/../.."
targets=("$@")
...
done < <(sed -n 's/.*("\([A-Z.]*\)" \. "\(BITCOIN-LISP[A-Z.-]*\)").*/\1 \2/p' src/package.lisp | tr 'A-Z' 'a-z')
rules=$(( ${#args[@]} / 2 ))
[ "$rules" -ge 10 ] || { echo "ERROR: could not read the nickname table from src/package.lisp" >&2; exit 1; }
sed -i '' "${args[@]}" \
  -e 's/(bl::install-package-nicknames)/(bitcoin-lisp::install-package-nicknames)/g' \
  $files
