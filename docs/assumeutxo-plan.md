...
| **P6** | **Interaction polish**: prune ranges (base+1 floor for unvalidated snapshot cs; halve `*prune-target-mib*` while historical exists), index rebind/restart on completion, `-reindex-chainstate` deletes snapshot cs, `dumptxoutset` rollback mode (via invalidateblock/reconsiderblock RAII) — rollback optional/deferrable | suite + targeted tests | M |
