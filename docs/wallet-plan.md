...
Date: 2026-07-15. Status (2026-08-22): **P0-P6 DONE** (P0 PR 283, P2 PR 288, P3 PR 290, P4 PR 293 per §5; P1, P5 and P6 shipped without their rows being updated — see `git log --grep="Wallet P"`). P7 (2026-09-24): every RPC Core's wallet registers (the 56 of `GetWalletRPCCommands`, wallet/rpc/wallet.cpp, at d3056bc) is registered here; G7-42's four -- `gethdkeys`, `setwalletflag`, `createwalletdescriptor` (commits "Track B P1: gethdkeys and setwalletflag", "Track B P1: createwalletdescriptor and estimaterawfee") and `migratewallet` ("Wallet: migratewallet exists, and answers Core's already-a-descriptor-wallet", Core's refusal only: no BDB here) -- are all in. Only the Sparrow acceptance run remains.
...
| **P7** | **DONE except Sparrow (2026-09-24)** **Polish/parity**: G7-42 = gethdkeys, setwalletflag, createwalletdescriptor, migratewallet (refusal only; Core's legacy import/dump RPCs are gone from Core itself at d3056bc, so nothing legacy is left to register); avoid_reuse flag, getreceivedbyaddress/label, listreceivedby*, listaddressgroupings, keypoolrefill, simulaterawtransaction, getwalletinfo field completeness; **Sparrow Wallet connected as external client** as an RPC-fidelity acceptance test | Sparrow connects, sees balances/history, composes+signs+broadcasts via our node | S-M |
