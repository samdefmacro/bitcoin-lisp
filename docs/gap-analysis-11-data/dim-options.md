...
| `-persistmempool` | `GetBoolArg(..., DEFAULT_PERSIST_MEMPOOL=true)` gates BOTH `LoadMempool` and `DumpMempool` (`node/mempool_persist_args.cpp:13`, `init.cpp:338,2047`) | both drive sites are unconditional (`src/node/init.lisp:833`, `src/node/shutdown.lisp:498`) | c | `-persistmempool=0`, the documented remedy for the ~45-minute replay outage, does nothing — finding `212b060f` |
| `-persistmempoolv1` | `GetBoolArg` → write the older on-disk format (`node/mempool_args.cpp:106`) | we always write Core's version 2 (`src/mempool/mempool.lisp:1413`) | c | an older node cannot read our dump; folded into finding `212b060f` |
